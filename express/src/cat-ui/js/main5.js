...
const createCatCards = (cats) => {
  ...
  cats.forEach((cat) => {
    ...
    img.src = cat.image || (cat.filename ? imageUrl + cat.filename + '_thumb' : '');
    img.alt = 'Image of a cat named ' + (cat.name || cat.cat_name);
    img.classList.add('resp');

    // open large image when clicking image
    img.addEventListener('click', () => {
      modalImage.src = cat.image || (cat.filename ? imageUrl + cat.filename : '');
      imageModal.alt = cat.name || cat.cat_name;
    ...
    h2.innerHTML = cat.name || cat.cat_name;
    ...
    li.appendChild(p3);
    ul.appendChild(li);
    if (user.role === 'admin' || user.user_id === cat.owner_id || user.user_id === cat.owner) {
      // add modify button
      const modButton = document.createElement('button');
      modButton.innerHTML = 'Modify';
      modButton.addEventListener('click', () => {
        const inputs = modForm.querySelectorAll('input');
        inputs[0].value = cat.name || cat.cat_name;
        ...
        if (user.role === 'admin') {
          ...
          modForm.querySelector('select').value = cat.owner_id || cat.owner;
        } else {
...
loginForm.addEventListener('submit', async (evt) => {
  ...
  try {
    ...
    console.log(e.message);
    alert('Kirjautuminen epäonnistui. Tarkista että backend on käynnissä.');
  }
});
